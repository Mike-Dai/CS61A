...
def move_stack(n, start, end):
    ...
    if n == 0:
        ...
        print_move(start, another)
        print_move(start, end)
        print_move(another, end)
    else:
        print_move(start, end)
        print_move(start, another)
        print_move(end, another)
        print_move(start, end)
        print_move(another, start)
        print_move(another, end)
        print_move(start, end)
        move_stack(n - 3, start, end)
...
def sub_interval(x, y):
    ...
    and any value in y."""
    "*** YOUR CODE HERE ***"
    tempx = interval(lower_bound(x), upper_bound(x))
    tempy = interval(-upper_bound(y), -lower_bound(y))
    return add_interval(tempx, tempy)
...
def quadratic(x, a, b, c):
    ...
    if middle <= left or middle >= right:
        low, high = f(left), f(right)
        return interval(min(low, high), max(low, high))
    else:
        extreme, low, high = f(middle), f(left), f(right)
        if a > 0:
            return interval(extreme, max(low, high))
        else:
            return interval(min(low, high), extreme)
